...
import transformer_utils as tr

import numpy as np
import os
if not os.path.exists('models'):
    os.makedirs('models')
# python3 train_transformer.py --dataset_file ten_tokens_singular_data.txt --vocabulary_file datasets/ten_tokens.txt --model_save test.mdl
...
num_heads = 2
model = tr.Transformer(n_src_vocab=len(vocabulary), \
            n_tgt_vocab=len(vocabulary), \
            len_max_seq=10*2, \
            d_word_vec=len(vocabulary), \
            d_model=len(vocabulary), \
            d_inner=4*len(vocabulary), \
            n_layers=1, \
            n_head=num_heads, \
            d_k=int(len(vocabulary)/num_heads), \
            d_v=int(len(vocabulary)/num_heads), \
            dropout=0.0, \
...
for epoch in range(args.num_epochs):
    ...
    for training_sequence_idx in training_sequence:
        ...
        X,y,pos = datasets[training_sequence_idx].get_datapoint(dataset_index, training=True, positions=True)
        X = torch.Tensor(X).reshape(args.batch_size, -1).long()
        y = torch.Tensor(y).reshape(args.batch_size, -1).long()

        X_pos = torch.Tensor(pos).reshape(args.batch_size, -1).long()
        y_pos = torch.Tensor([0]*len(y)).reshape(args.batch_size, -1).long()
    

        output = model(src_seq=X, src_pos=X_pos, tgt_seq=y, tgt_pos=y_pos)
