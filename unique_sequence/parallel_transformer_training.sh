...
do
	...
	do
		...
		do 
			...
			sbatch --time ${TIME} --job-name ${BASE_NAME}_${MODEL_ID} gru_training.sh ${BASE_NAME} ${MODEL_ID} ${NUM_EPOCHS} ${DATASET} ${VOCAB_FILE} ${NUM_LAYERS} ${NUM_HEADS} ${HIDDEN_SIZE} >> ${BASE_NAME}_training_ids.txt
		done
	done
done
